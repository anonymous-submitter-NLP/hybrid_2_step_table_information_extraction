...
def calculate_similarity(query: str, cell: str) -> float: 
    ...
    if abs(cell_quantity - query_quantity) < 1: 
        result =  1
    elif abs(cell_quantity - query_quantity) / query_quantity < 0.1:
        result = 0.9
    elif abs(cell_quantity - query_quantity) / query_quantity < 0.2:
        result = 0.8
    elif abs(cell_quantity - query_quantity) / query_quantity < 0.4:
        result = 0.6
    elif abs(cell_quantity - query_quantity) / query_quantity < 0.6:
        result = 0.4
    elif abs(cell_quantity - query_quantity) / query_quantity < 0.8:
        result = 0.2
    else:
        result = 0
